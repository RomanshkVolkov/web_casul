...
export const serializedImageUrl = (id: number) =>
   `${urlAPI}/storage/blobs/${id}.jpg?container=imagenes`;
   ...
   const typeSerializer = {
      ...
      family: (options: any) =>
         options
            .filter(
               (option: any) =>
                  (option.brandId === currentFilterValues.brand ||
                     currentFilterValues.brand === 0) &&
